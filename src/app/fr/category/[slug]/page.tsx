...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: `${category.name.fr} - ${seoConfig.siteName}`,
...
export default async function CategoryPage({ params }: Props) {
  ...
  const articles = getArticlesByCategory(slug, 'fr');
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          <Link href="/fr" className="text-2xl font-bold text-gray-900">
            {seoConfig.siteName}
          </Link>
          <nav className="flex gap-4">
            <Link href="/fr" className="text-gray-600 hover:text-gray-900">
              Accueil
            </Link>
            <Link href="/fr/blog" className="text-gray-600 hover:text-gray-900">
              Blog
            </Link>
          </nav>
        </div>
      ...
      <section className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-12">
        <div className="max-w-6xl mx-auto px-4">
          <h1 className="text-4xl font-bold mb-4">{category.name.fr}</h1>
